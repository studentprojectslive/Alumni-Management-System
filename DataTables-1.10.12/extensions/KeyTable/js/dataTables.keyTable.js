...
$.extend( KeyTable.prototype, {
	...
	_editor: function ( key, orig )
	{
		...
		// Return key should do nothing - for textareass it would empty the
		// contents
		if ( key === 13 ) {
			orig.preventDefault();
		}
	...
	/**
	 * Focus on a particular cell, shifting the tables paging if required
	 *
	...
	_focus: function ( row, column, shift )
	{
		...
		if ( lastFocus ) {
			// Dont trigger a refocus on the same cell
	 ...
	 *   table being blurred. This is so arrow keys wont blur the table, but
	 *   tab will.
	 * @private
	 */
	_shift: function ( e, direction, keyBlurable )
	{
		...
		// rows current position by the display start point
		if ( pageInfo.serverSide ) {
		...
		if ( row >= 0 && row < rows && $.inArray( column, columns ) !== -1
			...
			// No new focus, but if the table isnt blurable, then dont loose
			// focus
			e.preventDefault();
		}
		else {
...
KeyTable.defaults = {
	...
	/**
	 * Tab index for where the table should sit in the documents tab flow
	 * @type {integer|null}
	 */
	tabIndex: null
};
