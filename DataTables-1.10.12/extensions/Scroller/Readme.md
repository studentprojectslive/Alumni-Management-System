...
Note that rows in the table must all be the same height. Information in a cell which expands on to multiple lines will cause some odd behaviour in the scrolling. Additionally, the tables `cellspacing` parameter must be set to 0, again to ensure the information display is correct.
