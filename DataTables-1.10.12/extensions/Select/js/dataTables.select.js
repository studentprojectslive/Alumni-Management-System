...
DataTable.select.init = function ( dt ) {
	...
	// If the init options havent enabled select, but there is a selectable
...
be available via the API - so the end user wouldnt be able to select rows
...
function enableMouseSelection ( dt )
{
	...
	body
		.on( mousedown.dtSelect, selector, function(e) {
			...
			// element selection doesnt look terrible!
		...
		.on( click.dtSelect, selector, function ( e ) {
			...
			// If text was selected (click and drag), then we shouldnt change
			// the rows selected state
	...
	$(body).on( click.dtSelect, function ( e ) {
		if ( ctx._select.blurable ) {
			// If the click was inside the DataTables container, dont blur
			if ( $(e.target).parents().filter( dt.table().container() ).length ) {
				return;
			}

			// Dont blur in Editor form
...
function init ( ctx ) {
	...
	api.on( preXhr.dt.dtSelect, function () {
		// note that column selection doesnt need to be cached and then
...
apiRegister( select.style(), function ( style ) {
	...
	return this.iterator( table, function ( ctx ) {
		...
		// Add / remove mouse event handlers. They arent required when only
...
// this required that the table be in the document! If it isnt then something
