...
On IE <= 9, this is harder. The overrideMimeType trick doesnt work so we need
...
On IE > 9, overrideMimeType doesnt work but xhr2 does.
...
[request](https://github.com/mikeal/request) (but youre free to use your
favorite library !). If possible, download the file as a Buffer (you will get
better performances). If its not possible, you can fallback to a binary string
...
var req = http.get(url.parse("http://localhost/.../file.zip"), function (res) {
  ...
  // dont set the encoding, it will break everything !
