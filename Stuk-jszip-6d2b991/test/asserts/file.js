...
QUnit.module("file", function () {
    ...
    function str2blob (str) {
        ...
        try {
            // dont use an Uint8Array, see the comment on utils.newBlob
    ...
    JSZipTestUtils.testZipFile("add file: from XHR (with bytes > 255)", "ref/text.zip", function(textZip) {
        ...
        stop();
        zip.generateAsync({type:"binarystring"}).then(function(actual) {
            // high-order byte is discarded and wont mess up the result
            JSZipTestUtils.checkGenerateStability(actual);
            start();
        })[catch](JSZipTestUtils.assertNoError);
    });
    ...
    function testFileDataGetters (opts) {
        ...
        .then(function(zip) {
            ...
            opts.zip.file("file.txt", "changing the content after the call wont change the result");
            start();
        })[catch](JSZipTestUtils.assertNoError);
    ...
    test("Zip folder() shouldnt throw an exception", function() {
        var zip = new JSZip();
        try {
            zip.folder();
    ...
    test("folder() doesnt overwrite existing entries", function () {
        var referenceComment = "my comment";
        var zip = new JSZip();
        zip.file("folder", null, {
            dir : true,
        ...
        // calling folder() doesnt override it
        zip.folder("folder");
    ...
    test("createFolders works on a file", function () {
        ...
        ok(!zip.files['false/'], "the false/ folder doesnt exist");
    ...
    test("Folders can be avoided with createFolders", function() {
        ...
        zip.file("test/Readme", "Hello World!\n", {createFolders: false});
        ok(zip.files['test/Readme'], "the file exists");
        ok(!zip.files['test/'], "the folder doesnt exist");
    });
    ...
    test("Finding a file", function(assert) {
        ...
        zip.file("Readme.Pirate", "Ahoy mhearty!\n");
    ...
    test("Finding a file (text search) with a relative folder", function() {
        ...
        zip.folder("files").folder("translation").file("Readme.Pirate", "Ahoy mhearty!\n");
