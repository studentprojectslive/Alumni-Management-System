...
function testGenerate(options) {
    ...
    .then(function(zip) {
        ...
        zip.file("Hello.txt", "updating the zip file after the call wont change the result");
        return promise;
    })
...
testGenerateFor([{
    ...
    JSZipTestUtils.testZipFile("STORE doesnt compress, " + testName, file, function(expected) {
        testGenerate({
            prepare : function () {
                var zip = new JSZip();
