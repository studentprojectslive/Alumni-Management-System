...
(/** @lends <global> */function( $ ) {
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		else
		{
			...
			/* Dont require that the user must specify bRegex, bSmart or bCaseInsensitive */
			...
			if ( ! col.sType && col._sManualType ) {
				...
				for ( j=0, jen=types.length ; j<jen ; j++ ) {
					for ( k=0, ken=data.length ; k<ken ; k++ ) {
						...
						// Doesnt match, so break early, since this type cant
	...
	function _fnAddData ( oSettings, aDataIn, nTr, anTds )
	{
		...
		for ( var i=0, iLen=columns.length ; i<iLen ; i++ )
		{
			...
			// dont overwrite it, which might break bindings etc
			if ( nTr ) {
	...
	function _fnFeatureHtmlLength ( settings )
	{
		...
		// This split doesnt matter where _MENU_ is, we get three items back from it
	...
	function _fnScrollDraw ( settings )
	{
		...
		// Recalculate the sanity width - now that weve applied the required width,
	...
	/**
	 * Adjust a tables width to take account of vertical scroll bar
	 *  @param {object} oSettings dataTables settings object
	...
	var __row_selector = function ( settings, selector, opts )
	{
		return _selector_run( selector, function ( sel ) {
			...
			// Get nodes in the order from the `rows` array (cant use `pluck`) @todo - use pluck_order
			var nodes = [];
